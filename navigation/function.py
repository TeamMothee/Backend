...
def calculate_route_response(start, end, passList=None):
    """
...
def find_optimal_route(start, end, candidates, alpha=1.5):
    ...
    shortest_response = calculate_route_response(start, end)
    if not shortest_response:
        return None

    shortest_time = shortest_response["features"][0]["properties"].get("totalTime", float("inf"))

    # 후보 점이 2개일 경우 처리
    if len(candidates) >= 2:
        a, b = candidates[:2]  # 첫 두 후보를 a, b로 설정
        routes = [
            calculate_route_response(start, end, passList=[a]),
            calculate_route_response(start, end, passList=[b]),
            calculate_route_response(start, end, passList=[a, b]),
            calculate_route_response(start, end, passList=[b, a]),
        ]
        ...
        if optimal_response["features"][0]["properties"].get("totalTime", float("inf")) > alpha * shortest_time:
            return None
